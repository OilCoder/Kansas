...
This module handles the evaluation of the trained CNN model on the external test dataset. It includes functions for:
...
evaluate_model(model_path, preprocessor_path, external_test_data, curves_to_predict, selected_curves, unique_formations, best_params, save_plots_path=None):
    Purpose:
    ...
    Parameters:
        ...
        best_params (dict): Dictionary of the best hyperparameters found during optimization.
        save_plots_path (str, optional): Directory path to save the plots.
    Returns:
...
plot_well_logs(well_name, df, y_pred_df, curves_to_predict, save_plots_path):
    Purpose:
        Generates well log plots for the actual and predicted curves.
    Parameters:
        well_name (str): Name of the well.
        df (DataFrame): Original DataFrame containing the actual curves.
        y_pred_df (DataFrame): DataFrame containing the predicted curves.
        curves_to_predict (list): List of curves to predict.
        save_plots_path (str): Directory path to save the plots.
    Returns:
        None
    Comments:
        The function creates and saves plots comparing actual and predicted curves.
"""
...
def evaluate_model(
    ...
    # Get sequence length from best_params
    ...
    for well_name, df in external_test_data.items():
        ...
        X_test_reshaped, actual_sequence_length = reshape_data_for_cnn(X_test_processed, sequence_length)
...
def plot_well_logs(well_name, df, y_pred_df, curves_to_predict, save_plots_path):
    ...
    plt.suptitle(f'Well {well_name} - Actual vs Predicted Curves (CNN)', fontsize=16)
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    
    # Save the plot
    plot_file = os.path.join(save_plots_path, f'{well_name}_cnn_well_log.png')
