"""
Pipeline Script for CNN-based Machine Learning Workflow
------------------------------------------------------

This is the main orchestration script that coordinates the entire CNN-based machine learning workflow.
It manages data loading and preprocessing, hyperparameter optimization, model training, and evaluation
using the CNN model. This script acts as the entry point to run experiments and ties together all other modules.
...
1. Data Loading and Preprocessing:
    ...
    - Generate features.
    - Normalize and scale data using the preprocessor.

2. Hyperparameter Optimization:
    - Call `optimize_hyperparameters` from `optimizer.py` to find the best hyperparameters for the CNN model.

3. Model Training and Evaluation:
    - Use `train_final_model` from `trainer.py` to train the CNN model with the best hyperparameters.
...
- Misalignment of Data and Model:
    Ensure that the preprocessed data matches the input requirements of the CNN model.
...
---------
- Extensibility:
    The pipeline is designed to focus on the CNN model but can be extended in the future if other architectures
    are considered.
- Parallel Execution:
...
from src.cnn.optimizer import optimize_hyperparameters
...
def pipeline(data, selected_curves, curves_to_predict, unique_formations):
    """
    Main function to execute the CNN-based machine learning pipeline.
    ...
    # Step 0: Setup
    current_dir = os.path.dirname(__file__)  # This gives you 'code/src/cnn'
    log_dir = os.path.join(current_dir, 'files')  # This sets log_dir to 'code/src/cnn/files'
    log_file = os.path.join(log_dir, 'cnn_model.log')
    ...
    import logging
    logger = logging.getLogger(__name__)
    logger.info("Starting the CNN pipeline...")
    ...
    try:
        # Reduce the number of trials for faster execution
        study, top_configs = optimize_hyperparameters(X, y, preprocessor, n_trials=20)
        logger.info(f"    Initial hyperparameter optimization completed.")
    except Exception as e:
        logger.error(f"Error during hyperparameter optimization: {str(e)}")
        # Provide default hyperparameters as fallback
        logger.info("Using default hyperparameters due to optimization failure")
        default_params = {
            'num_conv_layers': 2,
            'filters': 32,
            'kernel_size': 2,
    ...
    # Provide paths to save the model and preprocessor
    model_dir = os.path.join(current_dir, 'model')
    os.makedirs(model_dir, exist_ok=True)
    save_model_path = os.path.join(model_dir, 'cnn_model.h5')
    save_preprocessor_path = os.path.join(model_dir, 'cnn_preprocessor.pkl')
    ...
    preprocessor_path = save_preprocessor_path
    save_plots_path = os.path.join(current_dir, 'plots')  # Directory to save plots
...
def main(data, selected_curves, curves_to_predict, unique_formations):
    """
    Main entry point for the cnn pipeline.
